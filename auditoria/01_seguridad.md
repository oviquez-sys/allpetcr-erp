...
1. **FRA-002** ✅ *implementado — commit `7ac5c84`*: `REVOKE UPDATE,
   DELETE` sobre `core_auditlog`, vía `manage.py endurecer_auditlog`.
   Verificado antes de implementar (no asumido): el rol de la app
   (`allpetcr`) no es superusuario — si lo fuera, el REVOKE no serviría de
...
2. **FRA-004** ✅ *implementado — commit `b499198`*: reporte diario
   ...
   de los cinco — una persona notando "esto no cuadra" detecta más fraude
   real que cualquier control técnico, y es casi gratis de construir.
   Automático vía `manage.py reporte_diario` (Programador de tareas,
   documentado en `PRODUCCION.txt`) y llega SIEMPRE a los destinatarios
   configurados, no solo a quien administra el sistema ese día — si
   dependiera de que alguien lo enviara a mano, sería la misma persona que
   podría "olvidarlo" el día que algo pasó.

**Nivel 2 — vale la pena, requiere una decisión, no bloqueante:**
3. **FRA-005** ⏳ *implementado — commit `bac4bec` — pendiente de
   verificación*. La "versión barata" original (mandarle una copia al
   otro socio) dejó de aplicar: con el servidor en DigitalOcean
   administrado por ambos socios, los dos ya tienen el mismo nivel de
   acceso. Se implementó en su lugar respaldo a Backblaze B2 con Object
   Lock (modo Compliance, 30 días) y una Application Key "Write Only" —
   verificado contra la documentación oficial de B2, no contra una cuenta
   real. **Condición de cierre**: confirmar contra la cuenta real de B2
   que (1) Object Lock efectivamente bloquea el archivo con 30 días de
   retención, y (2) la llave Write Only rechaza un intento de borrado.
4. **FRA-003** ✅ *implementado — commit `ee36944`*, con un alcance más
   directo que la versión barata que se había propuesto acá: en vez de
   solo notificar cuando alguien corre `restaurar` (lo que no evita el
   borrado, solo lo hace visible), el fix ataca la causa — `core_auditlog`
   de la base viva ya no se pisa con la del respaldo. La notificación
   automática a los socios cuando corre el comando queda como mejora
   complementaria, no implementada — sigue siendo válida si se quiere una
   segunda señal además de la preservación de la bitácora.
